...
export const TaskListTable:React.FC<Props> = ({ tableData }) => {
    const getStatusBadgeColor = (status: string) => {
        switch (status) {
            case 'Completed':
                return 'bg-green-100 text-green-500 border boder-green-200';
            case 'In Progress':
    ...
    const getPriorityBadgeColor = (priority: string) => {
        switch (priority) {
            case 'High':
                return 'bg-red-100 text-red-500 border boder-red-200';
            case 'Medium':
